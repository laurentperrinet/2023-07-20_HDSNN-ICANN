...
\begin{figure}%[t]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/izhikevich.pdf}% https://www.overleaf.com/5625872443qpcwrkssgbsf
    \caption{
...
The discussed model comes with certain limitations. First, one limitation is that the entire framework is based on discrete time binning, which is incompatible with the continuous nature of biological time. While this choice facilitated efficient implementation on conventional hardware like GPUs, it can be extended to a purely event-based SNN framework~\cite{grimaldi_robust_2023}. By analytically incorporating a precision term in the temporal value of input spikes, a purely event-based scheme can be achieved, promising speedups and energy gains in computations.

Second, the current model is purely feed-forward, meaning the spikes generated by postsynaptic neurons are solely based on information from their classical receptive fields. However, neural systems often involve lateral interactions among neurons in the same layer and feedback connections, which can be crucial for computational principles and modulating neural information. While our theoretical model can incorporate these recurrent connections by inserting new spikes into the list of spikes reaching presynaptic addresses, it requires proper tuning to prevent disruptions in the homeostatic state. For the implementation of predictive or anticipatory processes, recurrent activity would be essential, particularly when dealing with multiple different delays that require temporal alignment. Such recurrent activity has been previously modeled to explain phenomena like the flash-lag illusion. Implementing this using generalized coordinates and delay operators would enable the incorporation of predictive mechanisms within our proposed HD-SNN model, providing an elegant solution to this problem.

Addressing these limitations and exploring the extension of the HD-SNN model to event-based schemes and recurrent connections would enrich its potential applications and pave the way for better understanding neural information processing in complex systems.
%
