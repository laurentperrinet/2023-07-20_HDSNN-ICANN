...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
In recent studies, the importance of spike timing has been highlighted, as exemplified by the barn owl auditory system. In particular, the precise spike timing in both ears in response to the sound of a mouse enables the brain to determine the prey's position~\cite{goodman_spike-timing-based_2010}. This discovery aligns with a growing body of literature suggesting that the brain's dynamics often exhibit stereotyped sequences~\cite{grimaldi_precise_2023}.

To effectively detect spiking motifs, we propose a novel metric inspired by the \textit{polychronization} model developed by Izhikevich~\cite{izhikevich_polychronization_2006}. This theoretical model involves a random recurrent network of spiking neurons with biologically realistic synaptic delays and evolving weights governed by Spike-Time Dependent Plasticity (STDP) learning rule. 

The interplay between delays and STDP leads to the spontaneous organization of neurons into groups, which exhibit stereotypical time-locked firing patterns known as "polychronous groups." The polychronization model enables spiking neurons to self-organize into groups and generate patterns of reproducible time-locked firing. The STDP rule plays a crucial role in increasing synaptic weights for neurons involved in these polychronous activities, thereby consolidating the formation of such groups. 

Although neurons in these groups fire at different times, the heterogeneous delays allow their spikes to converge synchronously on the postsynaptic neuron, leading to the summation of excitatory postsynaptic potentials and resulting in the firing of the postsynaptic neuron (see Figure~\ref{fig:izhikevich}).

Overall, the polychronization model provides valuable insights into understanding spiking neural networks and their potential role in generating spatio-temporal spiking motifs. One limitation of this model is that the heterogeneous delays are fixed and cannot evolve over time. The fixed delays may limit its applicability to certain scenarios, but its mechanism offers valuable implications for studying neural activity patterns and their significance in the brain. 
